...
//ENum = Alle variabler i et objekt hvor alle er unike
...
function spawnBait() {
  ...
  const speed = 2;
  const bait = new TBait(spcvs, SpriteInfoList.food, pos, speed);
  GameProps.baits.push(bait);
  //Spawn a new bait every 0.5 to 1 seconds
  if (GameProps.status === EGameStatus.playing) {
...
function onKeyDown(aEvent) {
  switch (aEvent.code) {
    case "Space":
      if (GameProps.status === EGameStatus.playing && !GameProps.hero.isDead) {
        GameProps.hero.flap();
